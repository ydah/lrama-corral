  ...
  async init() {
    ...
    this.initPromise = (async () => {
      try {
        ...
        const bundleResponse = await fetch(`/lrama_bundle.rb?v=${Date.now()}`, {
          cache: 'no-store'
        });
        ...
        const railroadBundleResponse = await fetch(`/railroad_diagrams_bundle.rb?v=${Date.now()}`, {
          cache: 'no-store'
        });
        ...
        // Load lrama_api.rb
        const apiResponse = await fetch(`/lrama_api.rb?v=${Date.now()}`, {
          cache: 'no-store'
        });
